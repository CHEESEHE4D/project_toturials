...
import { GESTURE_ASSETS, getThemeAssets } from '../config/assets';
...
export default function PreparePage() {
  ...
  useEffect(() => {
    ...
    setError('');
    loadManifest(theme.id, controller.signal).then(() => {
      if (!alive.current) return;
      const assets = getThemeAssets(theme.id);
      const image = new Image();
      image.onload = () => { if (alive.current) setMediaReady(true); };
      image.onerror = () => { if (alive.current) setError('主题背景没有加载完成，请返回后再试。'); };
      image.src = assetUrl(assets.backgroundImage);
      if (assets.backgroundVideo) {
        const media = document.createElement('video');
        preloadVideo.current = media;
        media.preload = 'metadata'; media.playsInline = true; media.muted = true;
        media.src = assetUrl(assets.backgroundVideo); media.load();
      }
  ...
  const tutorialVideo = runtime.tutorialVideos?.[currentPose] ?? GESTURE_ASSETS[currentPose].videoUrl;
  const tutorialImage = GESTURE_ASSETS[currentPose].imageUrl;
  return <AppShell compact><div className="prepare-page" style={themeStyle(theme)}><div className="prepare-top" data-exit><Link to="/" className="back-link"><ArrowLeft size={18} /> 返回主题</Link><span className="selected-theme"><img src={posterUrl(theme.id)} alt="" />{theme.name}<span>· 约 90 秒</span></span></div>
    ...
    <section className="preparation-card" data-exit>
      ...
      {step === 3 && <><div className="tutorial-top"><span>动作 {poseIndex + 1} / 3</span><button className="text-button" onClick={() => setSkipDemo(true)}>跳过示范 <ChevronRight size={15} /></button></div><h1 tabIndex={-1}>{poseInfo[currentPose].name}</h1><p className="preparation-intro">{poseInfo[currentPose].hint}</p><div className="tutorial-rule-strip"><div>{poses.map(p => <span key={p} className={p === currentPose ? 'active' : ''}><PoseIcon pose={p} />{poseInfo[p].name}</span>)}</div><small>训练时按此顺序循环；每动作 3 秒，正确姿势累计满 2 秒即保持完成。</small></div><div className="tutorial-demonstration">{!skipDemo && tutorialVideo ? <video key={currentPose} src={assetUrl(tutorialVideo)} autoPlay loop muted playsInline onError={() => setSkipDemo(true)} /> : !skipDemo && tutorialImage ? <img src={assetUrl(tutorialImage)} alt={`${poseInfo[currentPose].name}教学姿势`} onError={() => setSkipDemo(true)} /> : <div className="tutorial-symbol"><PoseIcon pose={currentPose} /><span>{skipDemo ? '请在镜头前做出这个动作' : '动作示意图 · 正式教学素材待专业审核'}</span></div>}</div><div className="tutorial-attempt"><video ref={video} muted playsInline autoPlay aria-label="尝试动作的镜像预览" /><span>{recognition.pose === currentPose ? '看到了，轻轻保持一下' : recognition.pose === 'UNKNOWN' && recognition.tracking !== 'LOST' ? '请稍微调整手型' : `试着做一次${poseInfo[currentPose].name}`}</span></div><div className="tutorial-dots">{poses.map((p, i) => <span key={p} className={i <= poseIndex ? 'active' : ''} />)}</div><p className="micro-note">跳过示范后，仍需完成三个动作的真实识别确认</p></>}
